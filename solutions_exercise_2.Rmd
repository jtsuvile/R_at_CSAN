...
# The loop above only operates with two sexes (male, female). This is due to the fact that the data set we're working on only has male and female participants on it. If you have a data set with either missing gender values (NA) or provide the option "rather not say", "other" or similar, you might want to use something like the loop below which also uses the 'they' pronoun
