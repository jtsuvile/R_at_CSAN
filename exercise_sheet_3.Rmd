...
Please note that it's not always easy to decide how to get your data from its current state to tidy! If you feel a little overwhelmed with your own data set and how to get it to where it needs to be, you can send a bat signal to Juulia on Teams and we can have a quick look at your data together. In the mean time, you can complete these tasks with the example data set.

### Loading and inspecting data
If your own data set is already clean, you can also practice these steps with the exercise_data_messy.csv which is available on Teams. On the other hand, if you have raw data which needs cleaning, you can also do this on your own data set. The solutions are shown on the exercise data. You'll need to have the package tidyverse installed and loaded to work these exercises. See a handy cheat sheet for many of the commands we're using today [here](./images/data-import.pdf) 

Q3.1.1 Read in your data using `read_csv`. Make sure you have the correct number of columns and rows after the import. 

Tip: If you're having trouble with data import, you can also use the graphical "import dataset" (under the environment tab, in the top right window). 
...
Q3.1.3 Try to do that! In the example data set, I would argue that it makes most sense to have the data in a wide format (i.e. each emotion has its own column). There is also a second data file to add data from. 
...
Tip: Look for negative values for variables which cannot be negative, like age, or much higher values than what is feasible, like height over 300 or a negative weight. You can also try plotting your variables to observe very clear outliers.
...
Q3.1.7 See if you want to rename some columns with `rename` to make the names shorter (and therefore quicker to type). In the example data set, columns 7 to 11 could benefit from a shorter name.

Q3.1.8 Create a new column by combining values from existing columns with `mutate`. If you're working on the example data set, you can e.g. count the Body Mass Index (weight(kg) / height(m)^2) for the participants.

Note: Unlike I did in the demo, you should make sure you use height in meters and not height in cm to get sensible results.
...
Q3.1.10 Can you make a loop which goes through all of the columns in your data and prints out how many NA's there are in each column? You can use `sum(is.na(data[,columnname]))` to get the number of NAs in a specific column: `is.na()` returns a vector of logical values telling if the element at that spot is NA and since logical values are also 1s and 0s, we can use `sum()` to count how many TRUEs (i.e. NAs) there are in the vector.

Tip: Refer back to the loops you've worked with earlier to get help with the syntax and to make sure you have the right kinds of brackets at the right places. Remember to build your loop little by little - first you can try to get the loop to print out just the column name, then the column name and some text (e.g. 'NAs in <columnname>:') and then finally add the NA count.
...
Q3.2.3 Ask the visuals to show men and women separately by adding `fill=sex`.
...
Q3.2.7 Add title to the plot and axis labels with `labs(title="Age Distribution", x="Age", y = "Density")`
...
To superimpose a normal distribution curve, we can add `stat_function(fun = dnorm, n = 101, args = list(mean = mean(data$age, na.rm=T), sd = sd(data$age, na.rm=T)), colour='red')`

Q3.2.10 Now save the plot.

Tip: You can either put your `pdf()` and `dev.off()` calls around the whole thing, or you can first assign the whole plot call to a variable (so the first line starts with `p1 <- ggplot` etc.). You can also save plot via the graphical interface with "Export" (plots tab, lower right hand side), but I recommend you try to minimise the number of point-and-click operations you do with R. 
...
Think of a plot type you would like to do with your own data. Can you figure out how to do that in ggplot2? If you have made a plot from your own data with ggplot2, please share it on Teams > Random to show off and to motivate others!
